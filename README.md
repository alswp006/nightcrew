...
첫 실행은 **최근 14일**(`SESSIONS_BACKFILL_DAYS`)만 적립한다. 모든 이벤트의 `ts`는 적립 시각이라
상한이 없으면 켜는 날 원장이 과거 전부로 뒤덮이고 그날 개발일지가 몇 달치 요약이 된다.
제외된 세션도 나중에 다시 열면 그때부터 정상 적립된다 — 영구 제외가 아니다.
