...
- **Q7 (2주차 전)**: tailnet의 `desktop` 노드는 Windows 호스트인가 WSL2 게스트인가? (위성 rsync·ssh append 경로 결정 — v2.4: 위성 기기 무관)
...
| 3 | ~~위성→데스크톱 rsync~~ → v2.5 잠정 부재: 데스크톱→클라우드 미러 설정으로 대체 | 2주차 | — | desktop.crontab의 07:20 미러 줄 활성화 + 드라이브에 파일 확인 |
| 4 | ~~위성 deadman~~ → v2.5 잠정 부재: 07:00 스탠드업 미도착 = 이상 신호(사람-deadman). 맥미니 도입 시 복구 | 2주차 | — | 스탠드업이 매일 오는지 첫 주 확인 |
