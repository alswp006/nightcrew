# 야간조(Night Crew) 통합 설계 v2.4 — 최종 구현판
...
- **9.3** Sentinel-iOS(love_place): 웹 Playwright 스펙 부적용 — 배포 후 별도 설계. **실행 호스트 = M1**(v2.4 — 가끔 도는 작업이라 상시 서버 불필요, 재택/온디맨드 실행 후 §2.3 예외②로 적립). 이 문서에서는 자리만 예약.
